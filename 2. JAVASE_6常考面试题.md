...
1. new String()：总是在堆上创建字符串对象。
2. 通过字面量创建字符串：先尝试从常量池中获取，若不存在，则直接在常量池生成一个字符串并返回引用。
3. 字面量的+操作：在编译阶段合并，相当于通过字面量创建。
4. 变量的+操作：实际调用StringBuilder下的append()方法，会创建新的对象。
...
1. 执行`new`指令，在**堆**中为新的String对象申请空间。
2. 执行`ldc`指令，尝试从**常量池**中获取字符串`"abc"`，如果常量池中不包含该字符串，则在常量池中**新建一个字符串对象`"abc"`**。
3. 执行`invokespecial`指令，执行String对象的初始化过程。
...
```java
Integer i1 = 40; // 调用了 Integer.valueOf(2),使用常量池中的对象,没有创建新的对象
...
- 虚引用：无法通过虚引用获得对象，主要用来跟踪对象被垃圾回收的活动，必须配合引用队列使用。
...
![img](../../桌面/work/冬冬的校招指南(md版)/Java基础/Java基础知识点总结.assets/0f483f2437ce4ecdb180134270a00144~tplv-k3u1fbpfcp-watermark-8088782.image)
...
5.将key和value封装为Node插入到链表或红黑树后，在判断是否需要扩容，如果需要扩容，就结束put方法。
...
3、HashMap继承AbstractMap类；覆盖了hashcode() 和equals() 方法，以确保两个相等的映射返回相同的哈希值；**TreeMap继承SortedMap**类；他保持键的有序顺序；同时实现了NavigableMap接口，具备快速搜索集合内元素的能力。
...
### HashMap多线程操作有哪些隐患？HashMap扩容或者put时为什么要加锁
...
>
> ```java
> DirectByteBuffer buffer = ByteBuffer.allocateDirect(1024); // 分配1024字节的堆外空间，内部调用的是native方法
> buffer.putInt(100); // 把整型变量存储到堆外内存
> int value = buffer.getInt(); // 从堆外内存读取整型变量
> System.out.println(value);
> System.gc(); // 回收内存（不是必须）—— 通过虚引用回收
> ```
>
...
然后是动态分派，它表示需要在运行时，根据变量的动态类型来确定重写的方法——采用动态链接的方式，将符号引用转为直接引用。（多态 继承重写）

> 每个栈帧都包含一个符号引用，指向运行时常量池，需要被转为直接引用后才能找到栈帧所真正对应的方法。
...
4. CMS
    ...
    - 特点：并发、低停顿，但是容易产生内存碎片。

    > CMS通常作为老年代收集器，可与Serial或ParNew配合工作。

5. G1

    - 原理：Garbage-First收集器，不再有明确的分代区域，而是将堆空间化为为多个Region——每个Region都可以扮演老年代或新生代，通过一个优先级列表跟踪每个Region的回收价值，最后根据优先级进行Mixed GC。目标是**同时保证吞吐量和并发量**。

    - 算法：整体采用标记-整理法、局部采用标记-复制法。

        > 不会产生内存碎片，具体步骤与CMS类似，但是实现方式不同，最后回收的是Region。

    - 特点：JDK1.9默认使用 G1，适合于服务端。
